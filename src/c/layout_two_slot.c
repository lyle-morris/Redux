...
static uint32_t s_icon_resource_ids[2];
...
static uint32_t metric_resource(uint8_t metric) {
  return redux_metric_resource_64(metric);
}
...
static void battery_state_handler(BatteryChargeState state) {
  ...
  for (uint8_t i = 0; i < 2; ++i) {
    ...
    replace_slot_bitmap(i, redux_battery_resource_64_variant(state, redux_use_reverse_icons()));
  }
}
...
static void create_metric_slot(Layer *root_layer, uint8_t index, int16_t x) {
  ...
  s_value_layers[index] = create_text_layer(root_layer, GRect(x, TWO_SLOT_LABEL_Y, TWO_SLOT_LABEL_W, TWO_SLOT_LABEL_H), s_font_24, GTextAlignmentCenter, "", color);
